module Integrations::Csml::ProcessProductsMessage
  ...
  def process_add_labels(message_payload, conversation)
    AgentBots::ActionService.new(conversation.account, conversation).add_label(message_payload['content']['value'])
  end

  def process_remove_labels(message_payload, conversation)
    AgentBots::ActionService.new(conversation.account, conversation).remove_label(message_payload['content']['value'])
  end

  def process_assign_agent(message_payload, conversation)
    content = if AgentBots::ActionService.new(conversation.account, conversation).assign_agent([message_payload['content']['id'].to_i])
                "Assign agent id #{message_payload['content']['id']}"
              else
                "Assign agent id #{message_payload['content']['id']} failed"
              end
  ...
  def process_assign_team(message_payload, conversation)
    content = if AgentBots::ActionService.new(conversation.account, conversation).assign_team([message_payload['content']['id'].to_i])
                "Assign team id #{message_payload['content']['id']}"
              else
                "Assign team id #{message_payload['content']['id']} failed"
              end
